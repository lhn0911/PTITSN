...
export default function Friend() {
  ...
  const handleAccept = async (friendId) => {
    try {
      ...
      const updatedUser = {
        ...user,
        addfriend: user.addfriend.map((friend) =>
          friend.id === friendId ? { ...friend, status: true } : friend
        ),
      };

      // 2. Thêm bạn mới vào danh sách bạn bè
      const newFriend = user.addfriend.find((friend) => friend.id === friendId);
      updatedUser.friend = [
        ...(user.friend || []),
  ...
  const handleReject = async (friendId) => {
    try {
      ...
      const updatedUser = {
        ...user,
        addfriend: user.addfriend.filter((friend) => friend.id !== friendId),
      };
